---
description: Modified Ultracite Rules
globs: "**/*.{ts,tsx,js,jsx}"
alwaysApply: true
---
...
- Choose to derive data rather than use use Effect. Only use useEffect when you need to syncronize with an external system (e.g. document-level events). It causes misdirection of what the logic is going. Choose to explicitly define logic rather than depend on implicit reactive behavior
...
- `apps/web/web-context-profile.md` contains contextual information about the fronted TextPilot -`apps/server/server-context-profile.md` contains contextual information about the backend TextPilot
